...
class Screen {
 public:
  ...
  virtual ScreenNum Start(SimpleGUI *gui, SimpleVideo *video,
                          SimpleAudio *audio);
  virtual void Render(SimpleGUI *gui, SimpleVideo *video, SimpleAudio *audio);
  virtual ScreenNum Handle(SimpleGUI *gui, SimpleVideo *video,
                           SimpleAudio *audio, SDL_Event &event);
...
Screens::Screens() {
  ...
  video = new SimpleVideo(1024, 640, 0.0);
  ...
  cols.push_back(gui->NewColor(1.0, 0.0, 0.0));  // Temporary Model Has No
                                                 // Yellow
...
ScreenNum Screen::Start(SimpleGUI *gui, SimpleVideo *video,
                        SimpleAudio *audio) {
...
void Screen::Finish(SimpleGUI *gui) { gui->MasterWidget()->RemoveWidget(main); }

ScreenNum Screen::Handle(SimpleGUI *gui, SimpleVideo *video, SimpleAudio *audio,
                         SDL_Event &event) {
  return SCREEN_SAME;
}
...
void Popup::Finish(SimpleGUI *gui) { gui->UnsetPopupWidget(); }
...
ScreenNum Screen_Config::Start(SimpleGUI *gui, SimpleVideo *video,
                               SimpleAudio *audio) {
  Screen::Start(gui, video, audio);
  return SCREEN_SAME;
}

ScreenNum Screen_Config::Handle(SimpleGUI *gui, SimpleVideo *video,
                                SimpleAudio *audio, SDL_Event &event) {
...
Screen_Title::Screen_Title() {
  ...
  //  anims.push_back(guy->LookUpAnimation("LEGS_IDLE")); //Trooper is to
  //  jittery!
...
ScreenNum Screen_Title::Start(SimpleGUI *gui, SimpleVideo *video,
                              SimpleAudio *audio) {
  Screen::Start(gui, video, audio);
...
void Screen_Title::Render(SimpleGUI *gui, SimpleVideo *video,
                          SimpleAudio *audio) {
...
ScreenNum Screen_Title::Handle(SimpleGUI *gui, SimpleVideo *video,
                               SimpleAudio *audio, SDL_Event &event) {
  if (event.type == SDL_SG_EVENT) {
    if (event.user.code == SG_EVENT_BUTTONCLICK) {
      if (event.user.data1 == (void *)optb)
        return SCREEN_CONFIG;
      else if (event.user.data1 == (void *)multb)
        return SCREEN_MULTI;
      else if (event.user.data1 == (void *)singb)
        return SCREEN_SINGLE;
      else if (event.user.data1 == (void *)replb)
        return SCREEN_REPLAY;
      else if (event.user.data1 == (void *)quitb)
        return SCREEN_NONE;
    }
  }
  return SCREEN_SAME;
...
ScreenNum Screen_Single::Handle(SimpleGUI *gui, SimpleVideo *video,
                                SimpleAudio *audio, SDL_Event &event) {
  bool play_game = false, create_game = false;
  if (event.type == SDL_SG_EVENT) {
    if (event.user.code == SG_EVENT_BUTTONCLICK) {
      if (event.user.data1 == (void *)cancelb)
        return SCREEN_TITLE;
      else if (event.user.data1 == (void *)optb)
        return SCREEN_CONFIG;
      else if (event.user.data1 == (void *)loadb)
        return POPUP_LOADMAP;
      else if (event.user.data1 == (void *)contb)
        play_game = true;
  ...
  if (create_game) {
    ...
    for (int pn = 0; slot != conn.slots.end(); ++slot, ++pn) {
      if (slot->ptype == SC_PLAYER_LOCAL) {
        cur_game->AttachPlayer(
            new Player_Local(cur_game, PLAYER_LOCAL, pn, slot->color - 1));
      } else {  // if(slot->ptype == SC_PLAYER_AI) {
        cur_game->AttachPlayer(
            new Player_AI(cur_game, PLAYER_AI, pn, slot->color - 1));
      }
    }
    gob->Disable();
    contb->Enable();
...
ScreenNum Screen_Multi::Start(SimpleGUI *gui, SimpleVideo *video,
                              SimpleAudio *audio) {
  Screen::Start(gui, video, audio);
  if (!net_init) {
    if (SDLNet_Init() < 0) {
...
ScreenNum Screen_Multi::Handle(SimpleGUI *gui, SimpleVideo *video,
                               SimpleAudio *audio, SDL_Event &event) {
  if (event.type == SDL_SG_EVENT) {
    if (event.user.code == SG_EVENT_BUTTONCLICK) {
      if (event.user.data1 == (void *)cancelb)
        ...
        return SCREEN_CONFIG;
      else if (event.user.data1 == (void *)hostb)
        return POPUP_LOADMAP;
      else if (event.user.data1 == (void *)gob)
        return SCREEN_PLAY;
      ...
      //	    SDLNet_TCP_Close(slot->sock);	// Close it, no
      // networking yet
      //	    slot->sock = NULL;
      //	    }
      //	  }
...
ScreenNum Screen_Replay::Handle(SimpleGUI *gui, SimpleVideo *video,
                                SimpleAudio *audio, SDL_Event &event) {
  if (event.type == SDL_SG_EVENT) {
    if (event.user.code == SG_EVENT_BUTTONCLICK) {
      if (event.user.data1 == (void *)cancelb)
...
ScreenNum Screen_Results::Start(SimpleGUI *gui, SimpleVideo *video,
                                SimpleAudio *audio) {
  Screen::Start(gui, video, audio);
  return SCREEN_SAME;
}

ScreenNum Screen_Results::Handle(SimpleGUI *gui, SimpleVideo *video,
                                 SimpleAudio *audio, SDL_Event &event) {
  if (event.type == SDL_SG_EVENT) {
    if (event.user.code == SG_EVENT_BUTTONCLICK) {
      if (event.user.data1 == (void *)replb)
        return SCREEN_PLAY;
      else if (event.user.data1 == (void *)doneb)
        return SCREEN_TITLE;
      else if (event.user.data1 == (void *)quitb)
        return SCREEN_NONE;
      else if (event.user.data1 == (void *)saveb) {
      };  // FIXME: Implement!
    }
  }
  return SCREEN_SAME;
...
ScreenNum Screen_Play::Start(SimpleGUI *gui, SimpleVideo *video,
                             SimpleAudio *audio) {
  audio->Stop(cur_music);
  cur_music = -1;
  ...
  if (res == PLAY_FINISHED)
    return SCREEN_RESULTS;
  else if (res == PLAY_CONFIG)
    return SCREEN_CONFIG;
  else if (res == PLAY_SAVE)
    return SCREEN_RESULTS;  // For now
  else {
...
ScreenNum Screen_Play::Handle(SimpleGUI *gui, SimpleVideo *video,
                              SimpleAudio *audio, SDL_Event &event) {
  //  Do Nothing!
  return SCREEN_SAME;
}
...
ScreenNum Popup_LoadMap::Handle(SimpleGUI *gui, SimpleVideo *video,
                                SimpleAudio *audio, SDL_Event &event) {
  if (event.type == SDL_SG_EVENT) {
