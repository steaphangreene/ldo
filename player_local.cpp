  ...
  while (exiting == 0) {
    ...
    if (event.type == SDL_QUIT) {
      ...
      if (event.key.keysym.sym == SDLK_ESCAPE) {
        ...
        ++cur_zpos;
        if (cur_zpos >= percept.mapzs)
          --cur_zpos;
        else {
          SDL_mutexP(vid_mut);
          video->SetZPosition(CELL_HEIGHT * cur_zpos, MOVE_DELAY);
          scene->RestrictZ(
              0.0, CELL_HEIGHT * (cur_zpos + 1) - 1.5);  // FIXME: Real #
          SDL_mutexV(vid_mut);
      ...
      if (event.user.code == SG_EVENT_BUTTONCLICK) {
        ...
        if (event.user.data1 == (void *)(ednd)) {
          ...
          } else if (past && offset == 0 && playback_speed == 3 &&
        ...
        mouse_y = ((int)(y)) / 2;
        if (sel_id > 0 &&
            percept.UnitPresent(mouse_x, mouse_y, cur_zpos, targ_id) < 0) {
          raction = 2;
          ppass->SetMenu(3, ractions[raction]);
        } else if (sel_id > 0) {
          raction = 1;
        ...
        //		vars[0], vars[3], vars[4], vars[6], vars[1], vars[2],
        // vars[5]);
      ...
      //	}
      else if (event.user.code >= SG_EDGE_BEGIN &&
               event.user.code < SG_EDGE_END) {
...
void Player_Local::UpdateEquipIDs() {
  ...
  for (; unit != percept.my_units.end(); ++unit) {
    ...
    for (; act != unit->second.end(); ++act) {
      if (act->act == ACT_EQUIP && eqtmp.count(act->id) &&
          (targ == -1 || targ == act->targ1) && act->finish >= start_time &&
          act->finish < start_time + 3000) {
        targ = act->targ1;
  ...
  for (; id != eqid.end(); ++id) {
    ...
    ntab->SetBackground(
        new SG_Panel(gui->NewColor(49 / 255.0, 71 / 255.8, 49 / 255.0)));
