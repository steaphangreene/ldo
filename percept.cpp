...
int Percept::RDist(const MapCoord &start, const MapCoord &end) {
  ...
  if (dx == 0 && dy == 0 && dz == 0) {
    ...
    if (fabsf(height2 - height) > 1.0) {
      //      fprintf(stderr, "(%d,%d,%d)->(%d,%d,%d) is too big a step
      //      (%f->%f)\n",
      ...
      for (int zo = -1; zo < 2; zo++) {
        for (int yo = -1; yo < 2; yo++) {
          for (int xo = -1; xo < 2; xo++) {
            ...
            if (tmp.z >= 0 && tmp.z < mapzs && tmp.y >= 0 && tmp.y < mapys &&
                tmp.x >= 0 && tmp.x < mapxs && closed.count(tmp) == 0) {
              ...
              if (rd > 0) {
                ...
                while (test != end && tmp.z == cur.z) {  // FIXME: Z Straight?
                  ...
                  if (dir == pdir)
                    dirs[dir] = 1;  // Twice+ in a row
                  else
                    dirs[dir] = dirs[dir];  // Creates if missing
...
vector<MapCoord> Percept::GetPath2x2(const MapCoord &start,
                                     const MapCoord &end) {
  vector<MapCoord> ret;
  MapCoord cur = end;
  ret.push_back(cur);
  while (cur.x != start.x || cur.y != start.y || cur.z != start.z) {
    ...
    if (cur.y < start.y)
      cur.y++;
    else if (cur.y > start.y)
      cur.y--;
    if (cur.z < start.z)
      cur.z++;
    else if (cur.z > start.z)
      cur.z--;
    ret.push_back(cur);
  }
  return ret;
...
void Percept::FillActionsTo(int id, Uint32 f) {
  ...
  if (lasttime < f && (!my_units[id].empty())) {
    if (my_units[id].rbegin()->act == ACT_STAND) {
      ...
      See(id, f, my_units[id].rbegin()->x, my_units[id].rbegin()->y,
          my_units[id].rbegin()->z, my_units[id].rbegin()->angle, 20, 90.0);
    } else {
      AddAction(id, f, f - lasttime, my_units[id].rbegin()->x,
                my_units[id].rbegin()->y, my_units[id].rbegin()->z,
                my_units[id].rbegin()->angle, ACT_STAND,
...
void Percept::AddAction(int id, Uint32 f, Uint32 d, int xp, int yp, int zp,
                        float ang, Act a, int t1, int t2, int t3) {
...
void Percept::AddAction(const UnitAct &act) {
  ...
  See(act.id, act.finish, act.x, act.y, act.z, act.angle, 20, 90.0);
}

void Percept::See(int id, Uint32 tm, int xp, int yp, int zp, float ang,
                  int dist, float fov) {
  if (lastseen.count(id) > 0) {
    set<MapObject *>::iterator obj = lastseen[id].begin();
    for (; obj != lastseen[id].end(); ++obj) {
      (*obj)->Unsee(unplayer[id], tm);
    }
    lastseen.erase(id);
  }
  ...
  for (int z = -dist; z < +dist; ++z) {
    for (int y = -dist; y < +dist; ++y) {
      for (int x = -dist; x < +dist; ++x) {
        ...
        if ((z + zp) >= 0 && (z + zp) < mapzs && (y + yp) >= 0 &&
            (y + yp) < mapys && (x + xp) >= 0 && (x + xp) < mapxs &&
            sqrt(z * z + y * y + x * x) <= dist && ad <= (fov / 2.0)) {
          MapCoord pos = {x + xp, y + yp, z + zp};
          multimap<MapCoord, MapObject>::iterator obj = objects.find(pos);
          if (obj != objects.end()) {
            for (; obj != objects.upper_bound(pos); ++obj) {
              obj->second.See(unplayer[id], tm);
              lastseen[id].insert(&(obj->second));
            }
          }
        }
      }
