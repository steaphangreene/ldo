...
void World::SetViewAngle(int ang) { angle = ang; }
    ...
    if (!(obj->second.burn.empty())) {  // Check for Smoke and/or Fire
      ...
      if (burn > 0 && obj->second.type == GROUND_FLOOR) {  // Smoke and/or Fire
        for (Uint32 start = (percept->round - 2) * 3000;
          ...
          scene->SetParticlePosition(
              part, obj->first.x * 2.0 + 1.0 + float(rand()) / RAND_MAX / 2.0,
...
void World::AddModels(Uint32 offset) {
  ...
  for (; unitacts != percept->other_units.end(); ++unitacts) {
    ...
    for (; act != unitacts->second.end(); ++act) {
      if (act->act == ACT_MOVE || act->act == ACT_RUN ||
          act->act == ACT_START) {
        scene->MoveObject(unitmap[unitacts->first], act->x * 2 + 1,
                          act->y * 2 + 1, act->z * CELL_HEIGHT, act->finish,
                          act->duration);
        ...
        scene->TargetObject(unitmap[unitacts->first], act->targ1 * 2 + 1,
                            act->targ2 * 2 + 1, act->targ3 * CELL_HEIGHT,
                            act->finish - act->duration, 250);
        scene->ActObject(unitmap[unitacts->first], ACT_SHOOT, act->finish,
                         act->duration);
        scene->UnTargetObject(unitmap[unitacts->first], act->finish + 250, 250);
      } else if (act->act == ACT_EQUIP) {
        if (act->finish > 0) {  // Initial EQUIP is NOT Shown
          scene->ActObject(unitmap[unitacts->first], act->act, act->finish,
                           act->duration);
        }
      } else {
        scene->ActObject(unitmap[unitacts->first], act->act, act->finish,
...
void World::DrawOrders(Uint32 offset) {
  ...
  for (; ordit != orders->orders.end(); ++ordit) {
    ...
    if (ord->order == ORDER_MOVE) {
      ...
      glBegin(GL_QUADS);
      glVertex3f(xo + cos(ang + M_PI / 2) / 2, yo + sin(ang + M_PI / 2) / 2,
                 zof);
      glVertex3f(xt + cos(ang + M_PI / 2) / 2, yt + sin(ang + M_PI / 2) / 2,
                 ztf);
      glVertex3f(xt + cos(ang - M_PI / 2) / 2, yt + sin(ang - M_PI / 2) / 2,
                 ztf);
      glVertex3f(xo + cos(ang - M_PI / 2) / 2, yo + sin(ang - M_PI / 2) / 2,
...
void World::DrawSelBox(int sel_x, int sel_y, int sel_z, float r, float g,
                       float b) {
